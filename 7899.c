...

{
    ...
    for (i = 0; i < nCerts; i++) {
        ...
        requery:
            ...
            if (session->host_address_var) {

                if (!gnutls_x509_crt_check_host_address_var(cert, session->host_address_var)) {

                    error_setg(errp,

                               "Certificate does not match the host_address_var %s",

                               session->host_address_var);
