                    ...
                    abi_long arg8)
{
    ...
    case TARGET_NR_sethost_address_var:
        if (!(p = lock_user_string(arg1)))
            goto efault;
        ret = get_errno(sethost_address_var(p, arg2));
        unlock_user(p, arg1, 0);
...
#endif
#ifdef TARGET_NR_gethost_address_var
    case TARGET_NR_gethost_address_var:
    {
        char *name = lock_user(VERIFY_WRITE, arg1, arg2, 0);
        if (name) {
            ret = get_errno(gethost_address_var(name, arg2));
