...

{
    ...
    uint32_t status_flag_var, bitmask_var;
    ...
    for (y = strip->y1; y < strip->y2; y += 4)
    {
        ...
        for (x = strip->x1; x < strip->x2; x += 4)
        {
            ...
            if ((chunk_id & 0x01) && !(bitmask_var >>= 1))
            {
                ...
                status_flag_var = AV_RB32(data);

                data += 4;

                bitmask_var = 0x80000000;
            }

            if (!(chunk_id & 0x01) || (status_flag_var & bitmask_var))
            {

                if (!(chunk_id & 0x02) && !(bitmask_var >>= 1))
                {
                ...
                if ((chunk_id & 0x02) || (~status_flag_var & bitmask_var))
                {
                ...
                else if (status_flag_var & bitmask_var)
                {
